...
- Pipeline: prepare (version bump on `release/v<x.y.z>` branch) -> build -> auto-merge PR to main -> `npm publish --provenance --access public` (OIDC) + tag + `gh release create --generate-notes`
- Release notes auto-generated by GitHub from merged PR titles since the last tag
