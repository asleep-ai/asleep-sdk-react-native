...
- **`errorInfo` must stay in lockstep with `error`.** Every write or clear of `error` pairs an `errorInfo` write in the same `setState` — the classified object in `onTrackingFailed`, `null` everywhere else (action catches, success clears, `setError`). A stale `category` surviving a later unrelated error would misclassify it
